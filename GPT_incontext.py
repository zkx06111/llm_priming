...
def GPT_incontext(pref, suf,exposure_data_path,target_data_path,exposure_type,target_data_list,reverse,engine,temperature,response_path):
    ...
    for _,info in previous_exposure.iterrows():
        ...
        prompt += "Question1: " + Q1 + "\n(A)" + option1 + "\n(B)" + option2 + "\nAnswer:"
        completion_1 = openai.Completion.create(model=engine, prompt=prompt, max_tokens = 5, temperature = temperature, stop = ["\n"])
        text = completion_1.choices[0].text
        prompt += text + "\n"

        prompt += "Question2: " + Q2 + "\n(A)" + option3 + "\n(B)" + option4 + "\nAnswer:"
        completion_2 = openai.Completion.create(model=engine, prompt=prompt, max_tokens = 5, temperature = temperature, stop = ["\n"])
        text = completion_2.choices[0].text
        prompt += text + "\n"
